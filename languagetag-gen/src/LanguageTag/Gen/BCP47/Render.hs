...
module LanguageTag.Gen.BCP47.Render (renderSplitRegistry) where
...
import LanguageTag.BCP47.Registry (Deprecation (..))
...
import LanguageTag.Internal.BCP47.Syntax (BCP47 (NormalTag), Normal (..))
...
renderSubtagModuleWith tyname tydescription docnote proj sel reg =
  T.unlines $
    [ warning,
      ...
      "",
      "module LanguageTag.Internal.BCP47.Registry." <> tyname <> " where",
      "",
  ...
  T.unlines $
    moduleDoc
      ...
      <> [ "",
           ...
           "{-# OPTIONS_HADDOCK not-home #-}",
           "",
           "module LanguageTag.Internal.BCP47.Registry." <> tyname <> "Records",
           "  ("
             <> T.intercalate
               ", "
               [ lookupname1,
...
-- TODO: should really separate the file writing logic from the module rendering
-- logic
