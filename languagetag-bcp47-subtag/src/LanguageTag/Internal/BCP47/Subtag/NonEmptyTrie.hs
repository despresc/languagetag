...
module LanguageTag.Internal.BCP47.Subtag.NonEmptyTrie
  ( Trie (..),
    Node (..),
    Step (..),
...
insert l = flip modCreate l . go
  where
    go a (Just (Branch _ s m)) = Branch (Node a) s m
    go a _ = Leaf a
...
-- children, if it exists, returning 'Nothing' if that would result in
-- an empty 'Trie'.
prune :: [Subtag] -> Trie a -> Maybe (Trie a)
prune = updateSub $ const Nothing

-- | Delete the children of the given node, if it exists, returning
-- 'Nothing' if that would result in an empty 'Trie'
