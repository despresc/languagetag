...
module LanguageTag.BCP47.QuasiSpec (spec) where

import Data.Foldable (traverse_)
import Data.Text (Text)
import qualified Data.Text as T
import Test.Hspec
import LanguageTag.BCP47.Canonicalization (canonicalizeBCP47)
import LanguageTag.BCP47.Quasi (canontag, subtag, syntag, validtag)
import LanguageTag.BCP47.Registry (BCP47)
import LanguageTag.BCP47.Subtag (Subtag, PopError (..), popSubtagText)
import qualified LanguageTag.BCP47.Syntax as Syn
import LanguageTag.BCP47.Validation (validateBCP47)
...
spec :: Spec
spec = do
  let popSubtagText' t = case popSubtagText t of
        Left e -> Left e
        Right (st, t')
          | T.null t' -> Right st
          | -- unprincipled, but we don't need a finer distinction
            otherwise ->
            Left PopEmptySubtag
  let pSub = orExplode "failed to parse subtag" popSubtagText'
